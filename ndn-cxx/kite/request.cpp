...
Request::Request(Interest interest)
{
  ...
  if (interest.hasApplicationParameters()) {
    Block parameter = interest.getApplicationParameters();
    parameter.parse();
...
Request::extractPrefixes(const Interest& interest)
{
  ...
  if (it == name.begin()) {
    NDN_THROW(std::invalid_argument("RV prefix not found"));
  }

  if (it == name.end()) {
    NDN_THROW(std::invalid_argument("\"KITE\" keyword not found"));
  }
  ...
  if (producerSuffixLength > 0)
    ...
    NDN_THROW(std::invalid_argument("producer suffix not found"));
